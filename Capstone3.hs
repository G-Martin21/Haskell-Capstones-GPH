...
-- with partial application: cypher = ( `xor`encodingList), see function encoderDecoder
...
-- Note
-- https://www.perplexity.ai/search/what-is-the-Q4Rx92eNQzGZHXvJzLWyeQ?s=c
-- from Data.Bits, maxbits = finiteBitSize (maxBound :: Int) == 64
-- bits' = finiteBitSize (fromEnum (maxBound :: Char)) == 64 (so 64 bits is the size of the memory for Char and Int)
-- the input of the function intToBits' is Int
...
data StreamCipher = PRNG String

instance Cipher StreamCipher where
  encode (PRNG pad) text = applyOTP pad text
  decode (PRNG pad) text = applyOTP pad text

onePRNG :: StreamCipher
onePRNG = PRNG randomChars
  
randomBits = map intToBits (randomList 1337 7 100 12345)
randomChars = map bitsToChar randomBits   

randomList :: Int -> Int -> Int -> Int -> [Int]
randomList a b maxNumber seed = iterate (prng a b maxNumber) seed

---
genPRNG :: Int -> StreamCipher
genPRNG seed = PRNG (getChars seed)

getChars seed = map bitsToChar (getBits seed)
getBits seed = map intToBits (iterate (prng 1337 7 100) seed)

-- Example encode (genPRNG 72) "haskell", encode (genPRNG 84) "haskell", ...